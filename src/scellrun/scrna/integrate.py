...
def run_integrate(
    ...
    *,
    ...
    # Detect sample key if not provided (Seurat convention: orig.ident)
    if sample_key is None:
        for candidate in ("orig.ident", "sample", "batch", "donor"):
    ...
    # Cell-cycle scoring (optional regression target)
    cc_regressed = False
    ...
    # If we already log-normalized for CC scoring, don't redo it
    if not cc_regressed:
        sc.pp.normalize_total(adata, target_sum=1e4)
        sc.pp.log1p(adata)
    ...
    # Integration
    use_rep = "X_pca"
    if method == "harmony" and sample_key is not None and adata.obs[sample_key].nunique() > 1:
        try:
            sc.external.pp.harmony_integrate(adata, key=sample_key)
            use_rep = "X_pca_harmony"
        except (ImportError, AttributeError):
            # harmony not available — fall back to PCA
            pass
    elif method in ("rpca", "cca"):
        # scanpy ecosystem doesn't have direct rpca/cca; treat as a no-op
        # with a comment in the report. Real path: use scvi-tools or bbknn.
        pass
