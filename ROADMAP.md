...
| v0.3    | `scellrun scrna markers`  | Per-cluster differential markers across the resolution sweep (one csv per resolution).         | AIO stage 5 (`fam()`) + Rmd § 10 (`FindAllMarkers`) |
| v0.4    | `scellrun scrna annotate` | Two-tier annotation: (a) SingleR vs reference for broad cell type; (b) panel-match against profile-specific marker dict for fine subtype. PubMed evidence column embedded in report. | Rmd § 9-10                                           |
