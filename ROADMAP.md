...
| **v1.1.2** | SKILL.md deeper patch — codex 3rd-opinion findings (key naming sweep, multimodal/integrated/raw-count boundaries, tmux/disk/resume/secret hygiene, top-of-doc version-check ritual, profile-generic glossary structure) | Second doc-only patch chasing codex's deeper review of the v1.1.1 SKILL.md. (1) Decision-key naming swept against ground truth in source: the JSONL sample, panel auto-pick section, and source=user/auto explainer all now reference the actual code-emitted keys (`analyze.annotate.auto_panel` from `src/scellrun/analyze.py:976`, `annotate.panel` from `src/scellrun/scrna/annotate.py:425`); the SKILL.md-only paraphrase `panel_name` is removed from row examples and clarified as a `fix_payload` dict shape only. (2) "When NOT to use scellrun" gains explicit bullets for multimodal data (CITE-seq / ATAC / spatial), already-integrated h5ads (don't double-process), and missing raw counts (refuse if `.raw is None and 'counts' not in layers`). (3) Environment hygiene gains four subsections: long-running remote jobs (tmux / SLURM / nohup), disk-space and run-dir naming, resume / inspect a half-finished run, and secret handling for `ANTHROPIC_API_KEY`. (4) New top-of-doc "Version compatibility check" H2 placed right after Verification status so the agent runs the version check BEFORE any command lookup. (5) "Glossary: chondrocyte panel labels" renamed to "Profile-specific glossaries" with a placeholder note for future profiles. Frontmatter `min_scellrun_version` / `tested_against_version` bumped to 1.1.2; (v1.1.0) stale-version inline labels in the panel sections re-anchored to (v1.1.0+). No source code change. | this repo |
| **v1.2.0** ✓ | cross-disease profiles + views layer (single-ship) | Two surface additions in one minor bump. (1) Three new tissue profiles ship under `src/scellrun/profiles/`: `tumor` (TISCH 2.0 / Sun 2021 pan-cancer TME panel — TAMs, CAFs, T/B/NK/plasma, endothelial, neutrophils, plus a proliferation-marker malignant proxy; default thresholds), `brain` (Tasic 2018 / Hodge 2019 cortical-hippocampal panel — excitatory + GABAergic with PV/SST/VIP/LAMP5 broken out, oligo lineage, astrocytes, microglia, vasculature; mt% tightened to 10), `kidney` (KPMP / Stewart 2019 nephron + vascular + immune — PT, DT, TAL, CD principal/intercalated, podocytes, parietal, endothelial, mesangial, broad immune; mt% tightened to 15). All ship `celltype_broad` only — fine-subtype panels remain a joint-disease-only feature pending dedicated cold-validation. Registered in `_REGISTRY` so `scellrun profiles list` surfaces them. (2) New `06_views/` HTML index layer beside `05_report/` — `runlayout.STAGE_DIRS` gains `"views": "06_views"`. New `scellrun.views.build_views(run_dir)` emits three cross-sections from the same artifacts the main report references, via PURE relative-path HTML (no symlinks, no copies, no hardlinks): `by_resolution/<R>/index.html` (per-resolution UMAP grid + cluster sizes + top-5-per-cluster markers + annotations table when this resolution was annotated, footer link to full markers CSV); `by_cluster/cluster_<C>.html` (top-10 markers, panel + AI rationale, sample distribution from `cluster_by_sample.csv`, per-cluster cell-barcode CSV stub); `by_decision_source/index.html` (three lists of decisions grouped by `source=ai/user/auto` with boring-knob filtering on auto). `06_views/index.html` landing page links to all three. `analyze.py` calls `build_views` after the main report build (best-effort; skipped on exception so it never breaks the canonical 05_report deliverable); `analyze:views` row appended to `00_run.json`. EN + ZH `scrna_full_report` templates gain a top-of-page Views link. v1.2.1 (originally a separate bump for the views layer alone) is subsumed into this single minor release. | this repo |
| **v1.3.0** | review server + PDF export + cross-disease cold-validation | Three surface additions in one minor bump. (1) New `scellrun review <run-dir>` subcommand: single-file Flask app at `src/scellrun/review.py` (under 200 LOC business logic; HTML/JS templates inline) bound to 127.0.0.1 (never 0.0.0.0; `--port` falls forward 100 above when taken). UI exposes four override surfaces — cluster labels (text-input table pre-filled from `04_annotate/annotations.csv`), cell exclusions (textarea with first-5/last-5 preview), threshold sliders (max_pct_mt / max_genes / min_counts pre-filled from QC stage's effective values in the decision log), and a free-text notes field. Save POSTs to `/save`, writes `<run-dir>/06_views/review_overrides.json` with `schema_version=1`. `--read-only/--editable`, `--lang en|zh` (defaults to whatever `00_run.json`'s first stage used), and a status-text "saved at HH:MM:SS" feedback line. Flask added to main `dependencies` so `review` is a first-class subcommand. (2) New `scellrun analyze --apply-overrides <path>` flag: load the JSON, apply threshold overrides at QC (recorded as `source="user"` rows with the existing `user_thresholds_overrides` machinery), apply cluster_label_overrides post-annotate (rewrite `annotations.csv` with new `final_label` + `label_source` columns and drop one decision row per cluster), apply cell_exclusions by patching `04_annotate/annotated.h5ad`'s `obs.scellrun_qc_pass` to False (keeping the cells in-place, flag-don't-drop). Notes land as the top-level `review_notes` field on `00_run.json`. Meta-row `analyze.applied_overrides` carries the override file's path. (3) New `scellrun export <run-dir> --format pdf` subcommand: `src/scellrun/export.py` calls WeasyPrint over `05_report/index.html`. Default `--out` is `<run-dir>/05_report/index.pdf`; `--landscape/--portrait` (default portrait); `--format` plural-ready for future `markdown` / `epub`. WeasyPrint shipped under the `[export]` extra (`pip install scellrun[export]`); missing-dep error points the user at the install hint. Cross-disease cold-validation findings land in `docs/v1demo/cold_validation.md` under a new H2; panel tweaks (if any) live in the same commit with `# v1.3.0:` rationale comments. SKILL.md frontmatter bumps `min_scellrun_version` / `tested_against_version` to 1.3.0 and Decision-tree gains rows for `review` and `export`. | this repo |
| **v1.3.1** ✓ | close v0.7 dogfood silent-failure regressions | Three follow-ups: (1) `select_effective_sample_key` now considers column cardinality (≥2 unique levels) so single-sample inputs auto-degrade `--method` from harmony to none up front instead of dying inside integrate. (2) `markers` rank_genes_groups capped at `DEFAULT_RANK_GENES_N_PER_CLUSTER=500` to bound memory at large-cluster runs. (3) `integrate` subsets adata to HVGs before scale + PCA — `.X` payload drops ~95% on typical 30k+ cells × 33k+ gene runs, `.raw` retains full normalized expression for downstream markers/annotate. Decision row `integrate.scale_subset_to_hvg` carries actual gene counts. 5 new tests around the picker / panel auto-pick / decision-row presence. | this repo |
| **v1.3.2** | hardening: surface decision rationales + algorithm regression tests | Post-v1.3.1 audit pivots from feature work to hardening. (1) `chosen_resolution_for_annotate` and panel auto-pick rationales are now rendered in `05_report/index.html` "At a glance" — the user sees *why* `res=0.3` was picked over `res=0.5` (cluster counts + largest_pct + singletons) without grepping `00_decisions.jsonl`. (2) Self-check triggers in the at-a-glance render alongside their rationale string instead of bare `SC***` codes. (3) `tests/test_pick_best_resolution.py` (9 cases) and `tests/test_panel_autopick.py` (8 cases) add regression coverage to the two previously-untested heuristics — including the v0.7 BML_1 max-n_clusters trap and the v1.1.0 cold-validation 1.5× margin. (4) `__version__` synced to `pyproject.toml`. (5) CLAUDE.md Hard rule 4 rewritten from "v0.1 scope tight" to "v1.3 frozen surface — scRNA only" reflecting the actual frozen state and deferring multi-omics to a v2.0 milestone. No new public commands. | this repo |
| **deferred to v2.0** | conda-forge + registry image + multi-omics | conda-forge feedstock; tagged Docker push to ghcr.io; bulk RNA-seq subcommand (`scellrun bulk *`); metabolomics composite scoring; proteomics integration. **NOT planned for the v1.x series.** v1.3 froze the CLI surface around scRNA; expanding into bulk / multi-omics is a v2.0-scope rewrite (~40-60% of the scRNA path, per the 2026-05-07 architecture audit — needs new STAGE_DIRS registration, modality-aware Profile schema, new template trees, bulk fixtures). Until v2.0 is opened, the v1.x cycle is bug fixes + additive scRNA profiles only. | — |
